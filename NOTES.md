...
Maybe there should be a node graph where components are connected together, and you could connect things in the middle like

- **Approval component**:
  - Passes progress events through as their shape (i.e. ----- ----- ------ or something, maybe we can change Words type to have a "secret"/"sensitive" boolean that makes it show the shape even though it contains the actual text)
  - Withholds sentence events until you click a button to approve it, or discards them if you disapprove it
- **Text replacements** can be middleware components as well?

- Speech to Text UX: (implemented with Approval component?)
  ...
  - Button B is a Confirm button: once you have the preview of your words, you press Confirm to send them to output providers.
    - If you _don't_ want to say the words (i.e. if it got it wrong), you can either _tap_ Button A to cancel, or _hold_ button A and talk again (which will overwrite the words)
...
- [ ] Component: WebSpeechInput
- [ ] Component: AzureSpeechInput
- [ ] Component: WebSpeechOutput
- [ ] Component: AzureSpeechOutput
