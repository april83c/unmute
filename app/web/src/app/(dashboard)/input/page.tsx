...
const TIME_FORMAT = new Intl.DateTimeFormat(navigator.language, {
	hour: '2-digit',
	minute: '2-digit'
});
		...
		case 'success':
			if (activeModules.data.data != null) {
				const filtered = WEB_MODULES.filter((webModule) => {
					return activeModules.data.data.input.find(
						(i: any) => i.moduleId == webModule.id
					);
				});
					...
					<>
						<Tabs.Root defaultValue={filtered[0]?.id} mb="3">
							<Tabs.List mb="3">
								{filtered.map((f) => {
									return (
										<Tabs.Trigger value={f.id} key={f.id}>
											{f.friendlyName}
										</Tabs.Trigger>
									);
								})}
